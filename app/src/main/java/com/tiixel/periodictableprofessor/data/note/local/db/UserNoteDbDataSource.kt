...
class UserNoteDbDataSource @Inject constructor(
    ...
    override fun getUserNote(element: Byte): Single<GenericUserNote> {
        return Single.defer {
            ...
            Single.just(GenericUserNote(element = element, userNote = userNote?.user_note))
        }
    }
    ...
    override fun getUserNotes(): Single<List<GenericUserNote>> {
        return Single.defer {
            ...
            Single.just(userNotes.mapNotNull { GenericUserNote(element = it!!.element, userNote = it!!.user_note) })
        }
    }
